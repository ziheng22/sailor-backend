...
@router.get("", response_model=PaginatedResponse[ProjectOut], summary="项目列表", description="仅返回 is_displayed=true 的项目。`limit`/`offset` 分页。")
def list_projects(
    limit: int = Query(200, ge=1, le=500),
    offset: int = Query(0, ge=0),
    db: Session = Depends(get_db),
):
    q = db.query(Project).filter(Project.is_displayed.is_(True)).order_by(Project.sort_order, Project.id)
    total = q.count()
    return PaginatedResponse(items=q.offset(offset).limit(limit).all(), total=total, limit=limit, offset=offset)
...
@admin.get("", response_model=PaginatedResponse[ProjectOut], summary="项目列表（管理员）")
def admin_list_projects(
    limit: int = Query(200, ge=1, le=500),
    offset: int = Query(0, ge=0),
    db: Session = Depends(get_db),
):
    q = db.query(Project).order_by(Project.sort_order, Project.id)
    total = q.count()
    return PaginatedResponse(items=q.offset(offset).limit(limit).all(), total=total, limit=limit, offset=offset)
